...
@section('content')
            ...
            <div class="mb-5">
                ...
                <div class="chooser-container">
                    ...
                    @foreach($icons as $key => $file)
                        <div class="icon-box {{ Str::contains($goal->icon, str_replace('bw', '', $key)) ? 'active' : '' }}"
                            data-icon-key="{{ $key }}">
                            <img src="{{ asset('assets/images/' . $file) }}" alt="{{ $key }} Icon">
                        </div>
                    @endforeach
                </div>
                <input type="hidden" name="icon" id="iconInput" value="{{ $goal->icon }}">
            </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="chooser-container">
                        ...
                        @foreach($colors as $color)
                            <div class="color-swatch {{ $goal->color === $color ? 'active' : '' }}" data-color="{{ $color }}"
                                style="background-color: {{ $color }}">
                                <img src="{{ asset('assets/images/check.png') }}" class="check-icon" alt="Selected">
                            </div>
                        @endforeach
                    </div>
                    <input type="hidden" name="color" id="colorInput" value="{{ $goal->color }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initial selection
            let selectedIconKey = document.querySelector('.icon-box.active')?.dataset.iconKey || 'foodbw';
            let selectedColor = document.querySelector('.color-swatch.active')?.dataset.color || '#AA0000';
            ...
            function updateIcon() {
                if (iconColorMap[selectedIconKey] && iconColorMap[selectedIconKey][selectedColor]) {
                    iconInput.value = iconColorMap[selectedIconKey][selectedColor];
                }
            }
        });
